            ...
            <div className="bg-surface w-full max-w-xl rounded-3xl border border-white/10 shadow-2xl relative z-10 p-8 overflow-hidden">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-2 bg-primary/20 px-3 py-1 rounded-full border border-primary/30">
                        ...
                        <span className="text-xs font-black text-primary">{vibeTokens ? vibeTokens.toLocaleString() : 0}</span>
                    </div>
                </div>
                    ...
                    {[
                        ...
                        <div key={item.id} className="p-4 bg-white/5 border border-white/5 rounded-2xl hover:border-primary/50 transition-all group flex flex-col justify-between aspect-video">
                            <div className="flex justify-between">
                                ...
                                {purchasedEffects && purchasedEffects.includes(item.id) ? (
                            ...
                            <div>
                                ...
                                <button 
                                    disabled={vibeTokens < item.price || (purchasedEffects && purchasedEffects.includes(item.id))}
                                    onClick={() => {
                                        if (setVibeTokens) setVibeTokens(prev => prev - item.price);
                                        if (setPurchasedEffects) setPurchasedEffects(prev => [...prev, item.id]);
                                    }}
                                ...
                                >
                                    {purchasedEffects && purchasedEffects.includes(item.id) ? 'Active' : 'Buy Vibe'}
                                </button>
                            </div>
                        </div>
                    ))}
